...
def delete_non_main_branches(engine):
    ...
    # SELECT DOLT_BRANCH('-D', '<branch>'). '-D' force deletes just in
    ...
    with engine.connect() as conn:
        ...
        for row in results:
            ...
            stmt = text("SELECT DOLT_BRANCH('-D', '" + branch + "')")
            conn.execute(stmt)
...
def dolt_commit(engine, author, message):
    ...
    with engine.connect() as conn:
        ...
        for row in result:
            commit = row[0]
        if ( commit ): 
            print("Created commit: " + str(commit) )
...
def dolt_checkout(branch):
    engine_base = "postgresql+psycopg2://postgres:password@127.0.0.1:5432/sql_alchemy_big_demo"
